...
export async function register(req: Request<{ username: string, password: string }>, res: Response) {
    ...
    console.log(`Registered new user: ${created.username} with id: ${created.id}`);
    const sessionId = crypto.randomUUID()
    await db.userSessions.create({
        data: {
            sessionId,
    ...
    console.log(`Logging in new user: ${created.username} with sessionId: ${sessionId}`);
